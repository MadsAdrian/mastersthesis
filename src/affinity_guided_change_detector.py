...
from change_priors import affinity


class AffinityGuidedChangeDetector(ChangeDetectorMkII):
    def __init__(self, translation_spec, aff_lambda, reg_lambda, **kwargs):
        """
            Input:
        ...
        super(AffinityGuidedChangeDetector, self).__init__(translation_spec, **kwargs)
    ...
    def __call__(self, inputs, training=False):
        """
            ...
            Output:
                ...
                when training=True:
                    outputs - list with
                        A_x - tensor (None,hw,hw); affinity of x
                        A_y - tensor (None,hw,hw); affinity of y
                        A_x_hat - tensor (None,hw,hw,1); affinity of x_hat
                        A_y_hat - tensor (None,hw,hw,1); affinity of y_hat
        """
        x, y = inputs

        if training:
            x_hat, y_hat = self._fy(y, training), self._fx(x, training)
            A_x, A_y = affinity(x), affinity(y)
            A_x_hat, A_y_hat = affinity(x_hat), affinity(y_hat)

            retval = [A_x, A_y, A_x_hat, A_y_hat]
        else:
    ...
    def _train_step(self, x, y, clw):
        ...
        with tf.GradientTape() as tape:
            A_x, A_y, A_x_hat, A_y_hat = self((x, y), training=True)
            fx_loss = {
                "aff": self.loss_object(A_x, A_y_hat),
            ...
            fy_loss = {
                "aff": self.loss_object(A_y, A_x_hat),
...
if __name__ == "__main__":
    ...
    print("Initializing AffinityGuidedChangeDetector")
    cd = AffinityGuidedChangeDetector(TRANSLATION_SPEC, **LAMBDAS, **CONFIG)
    print("Training")
