...
def channels(dataset):
    return DATASET_CHANNELS[dataset]
...
def _clip(image):
    ...
    limits_u = tf.reduce_mean(temp, 0) + 3.0 * tf.math.reduce_std(temp, 0)
    limits_d = tf.reduce_mean(temp, 0) - 3.0 * tf.math.reduce_std(temp, 0)
    for i, limits in enumerate(zip(limits_d, limits_u)):
        channel = temp[:, i]
        channel = tf.clip_by_value(channel, 0, limits[1])
...
def fetch(name, patch_size=100, **kwargs):
    ...
    try:
        ...
        tf.print("Evaluating and saving prior")
