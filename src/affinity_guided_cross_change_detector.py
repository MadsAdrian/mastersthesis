...
class AffinityGuidedCrossChangeDetector(ChangeDetectorMkII):
    def __init__(self, translation_spec, **kwargs):
        ...
        """
        super(AffinityGuidedCrossChangeDetector, self).__init__(
    ...
    def _train_step(self, x, y, clw):
        ...
        with tf.GradientTape() as tape:
            aff_x, aff_y, x_hat, y_hat, aff_x_hat, aff_y_hat = self(
                (x, y), training=True
            )
            fx_loss = {
...
class UnpatchedAffinityGuidedCrossChangeDetector(AffinityGuidedCrossChangeDetector):
    def __init__(self, cross_lambda, aff_lambda, reg_lambda, **kwargs):
        """  Input as AffinityGuidedCrossChangeDetector  """
        self.lambdas = {
        ...
        super(AffinityGuidedCrossChangeDetector, self).__init__(**kwargs)
    ...
    def __call__(self, inputs, training=False):
        ...
        if training:
            ...
            retval = [aff_x, aff_y, x_hat, y_hat, aff_x_hat, aff_y_hat]
        else:
...
class PatchedAffinityGuidedCrossChangeDetector(AffinityGuidedCrossChangeDetector):
    def __init__(
        self, cross_lambda, aff_lambda, reg_lambda, affinity_patch_size, **kwargs
    ):
        """
            Input:
                affinity_patch_size - int, used in patched_affinity
                args as AffinityGuidedCrossChangeDetector
        """
        self.lambdas = {
            "cross": tf.constant(cross_lambda, dtype=tf.float32),
            "aff": tf.constant(aff_lambda, dtype=tf.float32),
...
if __name__ == "__main__":
    ...
    LAMBDAS = {
        ...
        "affinity_lambda": 1,  # weight for affinity loss term
    ...
    print("Initializing AffinityGuidedCrossChangeDetector")
    cd = AffinityGuidedCrossChangeDetector(TRANSLATION_SPEC, **LAMBDAS, **CONFIG)
    print("Training")
