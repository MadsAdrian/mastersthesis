...
def _affinity_reshape(images, patch_size):
    """
        Gridded reshape of image. Takes patches from h, w, c and stack them
        on the batch dimension.

        Only tested for images.shape[1] == images.shape[2]
        Does this have an inverse operation?

        patches = images.shape[1]//patch_size

        The first dimension of the output contains patches from
        images[0], ..., images[-1] in that order

        Input:
            images - tf.tensor [batches, h, w, c].
            patch_size - int
        Output:
            images - tf.tensor [batches*patches, patch_size, patch_size, c]
    """
    tmp = tf.image.extract_patches(
        images=images,
    ...
    return tf.reshape(tmp, [-1, patch_size, patch_size, images.shape[-1]])


@tf.function
def patched_affinity(tensor, patch_size):
    """
        Patched affinity computation
        Input:
            tensor - tf.tensor of shape [b, h, w, c]
            patch_size - int
        Output:
            tensor - tf.tensor [b*patches, patch_size^2, patch_size^2]
    """
    tensor_rs = _affinity_reshape(tensor, patch_size)
    return affinity(tensor_rs)
    ...
    Then it is normalized.
        Input:
            ...
            y - float, array of [batch_size, patch_size, patch_size, num_channels_y],
                Batch of patches from data domain y.
        Output:
            D - float, array of [batch_size, patch_size, patch_size], Degree matrix
    """
    ax = affinity(x)
    ay = affinity(y)
    ...
    it is normalized to be between the range [0,1] using max_norm.
        Input:
            ...
            y - float, array of [batch_size, patch_size, patch_size, num_channels],
                Batch of patches from data domain y.
        Output:
            ztz - float, array of [batch_size, patch_size, patch_size], Inner product
    """
    max_norm = x.shape[-1]
...
def patched_alpha(x, y, ps, pstr, **kwargs):
    ...
    bs = kwargs.get("affinity_batch_size", 250)
    assert x.shape[:2] == y.shape[:2]
    y_max, x_max = x.shape[:2]
    Alpha = np.zeros((y_max, x_max), dtype=np.single)
    covers = np.zeros((y_max, x_max), dtype=np.single)
    ...
    for i in trange(runs):
        ...
        alpha = alpha(batch_t1, batch_t2)
        for i in range(temp_idx.shape[0]):
            p = temp_idx[i, 0]
            q = p + ps
            r = temp_idx[i, 1]
            s = r + ps
            Alpha[p:q, r:s] += alpha[i]
            covers[p:q, r:s] += 1
        idx = idx[bs:]
...
def tf_patched_alpha(x, y, ps, pstr):

    sizes = [1, ps, ps, 1]
    strides = [1, pstr, pstr, 1]
    rates = {
        "zoom": [1, 0.5, 0.5, 1],
        "static": [1, 1, 1, 1],
        "reduce2^1": [1, 2, 2, 1],
...
def eval_prior(name, x, y, **kwargs):
    ...
    It saves the intermediate results and the final result into a png and returns Alpha
        ...
        Output:
            Alpha - float, array of [image_height, image_width], Alpha prior
    """
    sizes = kwargs.get("sizes", [1, 2, 3])
    new_dims = np.array(x.shape[:-1])
    D = np.zeros((x.shape[0], x.shape[1], sizes))
    ...
    for i in sizes:
        ...
        write_image_to_png(d, "images/Affinities/" + name + "/aff_" + str(i) + ".png")
        D[..., ch] = d[..., 0]
        ch += 1
    D = np.mean(D, axis=-1, keepdims=True)
    write_image_to_png(D, "images/Affinities/" + name + "/aff.png")
    return D
